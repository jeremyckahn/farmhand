...
export const processCellar = (state: farmhand.state) => {
  ...
  for (let i = 0; i < newCellarInventory.length; i++) {
    ...
    // Only wine and vinegar are worth calling out here - they're a
    // deliberate long-term investment (kegs that never spoil and
    // appreciate in value). Generic fermented crops still spoil if left
    // too long, so their own spoilage notification already covers that
    // urgency without needing a separate "ready" notification too.
    const isNotifiableKeg =
